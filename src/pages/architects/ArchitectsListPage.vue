...
const sort = ref('fullName,asc')
const architects = ref<Architect[]>([])
const sortOptions = [
  { value: 'fullName,asc', label: 'Ім\'я А → Я' }, { value: 'fullName,desc', label: 'Ім\'я Я → А' },
  { value: 'experienceYears,desc', label: 'Досвід ↓' }, { value: 'experienceYears,asc', label: 'Досвід ↑' },
]
...
<template>
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
      <div>
        <h1 class="text-3xl font-bold text-slate-900">Архітектори</h1>
        <p class="mt-1 text-sm text-slate-500">{{ pagination.totalElements.value }} архітекторів</p>
      </div>
    ...
    <div class="mb-6">
      <FilterPanel :has-active-filters="hasActiveFilters" @clear="clearFilters">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <BaseInput v-model="filters.fullName" label="Ім'я" placeholder="Пошук за ім'ям..." />
          <BaseInput v-model="filters.specialization" label="Спеціалізація" placeholder="Пошук..." />
          <BaseInput v-model="filters.minExperience" label="Досвід від (р.)" type="number" placeholder="0" />
          <BaseInput v-model="filters.maxExperience" label="Досвід до (р.)" type="number" placeholder="50" />
        </div>
      </FilterPanel>
    </div>
    ...
    <EmptyState v-else-if="architects.length === 0" title="Архітекторів не знайдено" />
