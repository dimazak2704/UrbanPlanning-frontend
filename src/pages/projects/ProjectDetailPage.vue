      ...
      <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="sticky top-16 z-40 -mx-4 px-4 py-3 bg-slate-50/90 backdrop-blur-md sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 mb-6 border-b border-slate-200">
          <nav class="flex items-center gap-1.5 text-sm text-slate-500 flex-wrap">
            ...
            <RouterLink to="/projects" class="hover:text-slate-700">{{ t('header.projects') }}</RouterLink>
            <ChevronRightIcon class="h-3.5 w-3.5" />
        ...
        <div v-if="canEdit" class="flex gap-2 mb-6">
          ...
          <BaseButton variant="danger" size="sm" @click="showDeleteModal = true"><template #iconLeft><TrashIcon class="h-4 w-4" /></template>{{ t('common.delete') }}</BaseButton>
        </div>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3 mb-10">
          <div class="lg:col-span-2 space-y-6">
            <div v-if="project.description" class="rounded-xl border border-slate-200 bg-white p-6">
              <h2 class="text-lg font-semibold text-slate-900 mb-3">{{ t('projects.form.description') }}</h2>
              <p class="text-sm text-slate-600 leading-relaxed whitespace-pre-line">{{ project.description }}</p>
            </div>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div class="rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-center gap-1.5 text-xs text-slate-500 mb-1"><CurrencyDollarIcon class="h-4 w-4" />{{ t('projects.form.budget') }}</div>
                <p class="text-lg font-bold text-slate-900">{{ formatCurrency(project.budget) }}</p>
              </div>
              <div class="rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-center gap-1.5 text-xs text-slate-500 mb-1"><CalendarIcon class="h-4 w-4" />{{ t('projects.form.startDate') }}</div>
                <p class="text-lg font-bold text-slate-900">{{ formatDate(project.startDate) }}</p>
              </div>
              <div class="rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-center gap-1.5 text-xs text-slate-500 mb-1"><CalendarIcon class="h-4 w-4" />{{ t('projects.form.endDate') }}</div>
                <p class="text-lg font-bold text-slate-900">{{ formatDate(project.endDate) }}</p>
              </div>
              <div class="rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-center gap-1.5 text-xs text-slate-500 mb-1"><MapPinIcon class="h-4 w-4" />{{ t('projects.location') }}</div>
          ...
          <div class="space-y-4">
            <RouterLink :to="`/architects/${project.architectId}`" class="block rounded-xl border border-slate-200 bg-white p-5 hover:shadow-md transition-shadow">
              <div class="flex items-center gap-1.5 text-xs text-slate-500 mb-2"><UserIcon class="h-4 w-4" />{{ t('projects.form.architect') }}</div>
        ...
        <div>
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-semibold text-slate-900">{{ t('projects.infrastructures', { count: infras.length }) }}</h2>
      ...
      <teleport to="body">
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div class="mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 class="text-lg font-semibold text-slate-900 mb-2">{{ t('projects.deleteConfirmTitle') }}</h3>
            <p class="text-sm text-slate-500 mb-6">{{ t('projects.deleteConfirmDesc') }}</p>
